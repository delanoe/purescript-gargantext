...
newtype Frontends
  = Frontends { app :: Frontend, static :: Frontend }
...
sessionPath (R.Children n o l s i) = root <> "children?type=" <> show n <> offsetUrl o <> limitUrl l <> orderUrl s
  where
  root = sessionPath (R.NodeAPI Node i) <> "/"
...
sessionPath (R.ListDocument lId dId) = sessionPath (R.NodeAPI NodeList lId) <> "/document/" <> (show $ maybe 0 identity dId)
...
orderByUrl = maybe "" (\x -> "&orderBy=" <> show x)
 -- nodeTypePath :: NodeType -> Path -- nodeTypePath = NodeAPI -- instance toUrlNodeType :: ToUrl NodeType where --   toUrl ec e nt i = toUrl ec e (NodeAPI nt) i -- instance toUrlPath :: ToUrl Path where --   toUrl ec e p i = doUrl base path params --     where --       base   = endBaseUrl e ec --       path   = endPathUrl e ec p i --       params = "" ------------------------------------------------------------