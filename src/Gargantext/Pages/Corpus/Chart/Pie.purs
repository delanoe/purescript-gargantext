...
chartOptionsBar (HistoMetrics { dates: dates', count: count' }) =
  Options
    { mainTitle: "Bar"
...
chartOptionsPie (HistoMetrics { dates: dates', count: count' }) =
  Options
    { mainTitle: "Pie"
...
getMetrics session { corpusId, tabType: tabType } = do
  ...
  pure ms."data"
  where
  chart = Chart { chartType: ChartPie, tabType: tabType } (Just corpusId)
