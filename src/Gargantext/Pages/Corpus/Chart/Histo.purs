...
type Path
  = { corpusId :: Int, tabType :: TabType }

type Props
  = ( path :: Path, session :: Session )

newtype ChartMetrics
  = ChartMetrics { "data" :: HistoMetrics }
...
newtype HistoMetrics
  = HistoMetrics { dates :: Array String, count :: Array Number }
...
chartOptions (HistoMetrics { dates: dates', count: count' }) =
  Options
    { mainTitle: "Histogram"
...
getMetrics session { corpusId, tabType } = do
  ...
  pure ms."data"
  where
  chart = Chart { chartType: Histo, tabType: tabType } (Just corpusId)
