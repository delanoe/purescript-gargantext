...
type DocPath
  = { nodeId :: Int, listIds :: Array Int, corpusId :: Maybe Int, tabType :: TabType }
...
instance decodeStatus :: DecodeJson Status where
  decodeJson json = do
    obj <- decodeJson json
    failed <- obj .: "failed"
...
instance decodeDocumentV3 :: DecodeJson DocumentV3 where
  decodeJson json = do
    obj <- decodeJson json
...
instance decodeDocument :: DecodeJson Document where
  decodeJson json = do
    obj <- decodeJson json
    abstract <- obj .:? "abstract"
    authors <- obj .:? "authors"
...
docViewSpec = simpleSpec performAction render
  ...
  performAction (SetTermListItem n pl) { path: { nodeId, listIds, tabType }, session } { ngramsVersion } = commitPatch session { nodeId, listIds, tabType } (Versioned { version: ngramsVersion, data: pt })
    where
    pe = NgramsPatch { patch_list: pl, patch_children: mempty }

    pt = singletonNgramsTablePatch CTabTerms n pe

  performAction (AddNewNgram ngram termList) { path: { nodeId, listIds, tabType }, session } { ngramsVersion } = commitPatch session { nodeId, listIds, tabType } (Versioned { version: ngramsVersion, data: pt })
    where
    pt = addNewNgram CTabTerms ngram termList

  render :: Render State Props Action
  render dispatch { loaded: { ngramsTable: Versioned { data: initTable }, document } } { ngramsTablePatch } _reactChildren =
    [ autoUpdateElt
        { duration: 3000
        ...
        [ div [ className "row" ]
            [ div [ className "col-md-8" ]
                ...
                , ul [ className "list-group" ]
                    ...
                    , li'
                        [ span [] [ text' doc.publication_date ]
                        , badge "date"
                        ]
                    ]
                , badge "abstract"
...
type LayoutProps
  = ( session :: Session, nodeId :: Int, listId :: Int, corpusId :: Maybe Int )
...
documentLayoutCpt = R.hooksComponent "G.P.Corpus.Document.documentLayout" cpt
  ...
  cpt { session, nodeId, listId, corpusId } _ = do
    useLoader path (loadData session)
      $ \loaded ->
