...
annuaireCpt = R.staticComponent "G.P.Annuaire.annuaire" cpt
  where
  cpt { session, path, info: info@(AnnuaireInfo { name, date: date' }) } _ =
    R.fragment
...
contactCells session = maybe [] render
  ...
  render (Contact { id, hyperdata: (HyperdataContact contact@{ who: who, ou: ou }) }) =
    let
      nodepath = NodePath NodeContact (Just id)

      href = url session nodepath
    in
      [ H.text ""
  ...
  contactWhereOrg (ContactWhere { organization: orga }) = maybe "No orga (list)" identity (head orga)

  contactWhereDept (ContactWhere { labTeamDepts: [] }) = "Empty Dept"

  contactWhereDept (ContactWhere { labTeamDepts: dept }) = maybe "No Dept (list)" identity (head dept)

  contactWhereRole (ContactWhere { role: Nothing }) = "Empty Role"

  contactWhereRole (ContactWhere { role: Just role }) = role

data HyperdataAnnuaire
  = HyperdataAnnuaire
    { title :: Maybe String
    , desc :: Maybe String
    }
...
newtype AnnuaireTable
  = AnnuaireTable { annuaireTable :: Array (Maybe Contact) }
...
loadPage session { nodeId, params: { offset, limit, orderBy } } = get $ url session children
  -- TODO orderBy
  -- where
