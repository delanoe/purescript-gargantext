...
module Gargantext.Utils.KarpRabin
  ( -- * Overview -- $overview
  -- ** Caution
  -- $caution
  -- * Function
...
indicesOfAny pats =
  if A.null nepats then
    const []
  else
    strictMatcher nepats
  where
...
rehash' shDi out h o n = (h `shl` shDi - (fromCodePoint o `shl` out)) + fromCodePoint n

minimum1 :: forall a f. Ord a => Foldable f => a -> f a -> a
minimum1 a fa = case minimum fa of
  Nothing -> a
  Just b -> min a b
...
strictMatcher pats = unsafePartial search
  ...
  hash :: String -> UInt
  hash =
    foldl (\h w -> (h `shl` fromInt shDi) + fromCodePoint w) (fromInt 0)
      <<< S.toCodePointArray
      <<< S.take hLen
  ...
  search str =
    ...
    go sI h = case M.lookup h hashMap of
      Nothing ->
        if sI == maxIdx then
          L.Nil
        else
      ...
      Just ps ->
        let
          ...
          okay bs = isJust (S.stripPrefix (S.Pattern bs) rst)
        in
