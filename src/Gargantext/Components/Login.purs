...
modalCpt = R.hooksComponent "Modal" cpt
  ...
  cpt { visible } children = do
    ...
    elems =
      [ H.div
          { id: "loginModal"
          , className: modalClass (fst visible)
          , role: "dialog"
          , "data": { show: true }
          , style: { display: "block" }
          }
...
renderSessions sessions = render (unSessions $ fst sessions)
  where
...
renderBackend backend@(Backend { name }) state = H.li {} [ H.a { on: { click } } [ H.text $ "Connect to " <> name ] ]
  where
...
formCpt = R.hooksComponent "G.C.Login.form" cpt
  where
  ...
  onClick { backend, sessions, visible } error username password e =
    launchAff_
      $ do
          let
            req = AuthRequest { username: fst username, password: fst password }
...
termsCheckbox _ = H.input { id: "terms-accept", type: "checkbox", value: "", className: "checkbox" }

termsLink :: {} -> R.Element
termsLink _ = H.a { target: "_blank", href: termsUrl } [ H.text " [ Read the terms of use ] " ]
  where
  termsUrl = "http://gitlab.iscpif.fr/humanities/tofu/tree/master"

requestAccessLink :: {} -> R.Element
requestAccessLink _ = H.a { target: "_blank", href: applyUrl } [ H.text " request access" ]
  where
  applyUrl = "https://iscpif.fr/services/applyforourservices/"
...
loginSubmit click = H.button { id, className, type: "submit", on: { click } } [ H.text "Login" ]
  where
  id = "login-button"
