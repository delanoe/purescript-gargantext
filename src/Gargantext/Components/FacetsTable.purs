...
newtype SearchResults
  = SearchResults { results :: Array Response }
          ...
          [ H.div { className: "row" }
              ...
              , H.div { className: "col-md-12" }
                  [ H.button { style: buttonStyle, on: { click: trashClick deletions } }
                      [ H.i
                          { className: "glyphitem glyphicon glyphicon-trash"
...
performDeletions session nodeId (deletions /\ setDeletions) = launchAff_ call *> setDeletions del
  where
...
markCategory session nodeId category nids = void $ launchAff_ $ putCategories session nodeId (CategoryQuery q)
...
togglePendingDeletion (_ /\ setDeletions) nid = setDeletions setter
  where
  setter deletions@{ pending } = deletions { pending = toggleSet nid pending }
...
docViewGraphCpt = R.hooksComponent "FacetsDocViewGraph" cpt
  ...
  cpt { session, nodeId, listId, query, totalRecords, chart, container } _ = do
    ...
    let
      performClick = \_ -> performDeletions session nodeId deletions
    path <- R.useState' $ initialPagePath { nodeId, listId, query, session }
    pure
      $ R.fragment
          [ H.br {}
...
type PagePath
  = { nodeId :: Int, listId :: Int, query :: TextQuery, params :: T.Params, session :: Session }
...
loadPage { session, nodeId, listId, query, params: { limit, offset, orderBy } } = do
  ...
  let
    url2 = url session $ Search { listId, offset, limit, orderBy: convOrderBy <$> orderBy } (Just nodeId)
...
type PageProps
  = ( documents :: Array DocumentsView | PageLayoutProps )
  ...
  cpt { totalRecords, container, deletions, documents, session, path: path@({ nodeId, listId, query } /\ setPath) } _ = do
    ...
    pairUrl (Pair { id, label })
      | id > 1 = H.a { href, target: "blank" } [ H.text label ]
        where
        href = url session $ NodePath NodeContact (Just id)
...
---------------------------------------------------------
newtype DeleteDocumentQuery
  = DeleteDocumentQuery { documents :: Array Int }

instance encodeJsonDDQuery :: EncodeJson DeleteDocumentQuery where
  encodeJson (DeleteDocumentQuery post) = "documents" := post.documents ~> jsonEmptyObject

deleteDocuments :: Session -> Int -> DeleteDocumentQuery -> Aff (Array Int)
deleteDocuments session nodeId =
  deleteWithBody
    $ (url session $ NodeAPI Node $ Just nodeId)
    <> "/documents"
