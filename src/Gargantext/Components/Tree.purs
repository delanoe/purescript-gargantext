...
filterNTree p (NTree x ary) = NTree x $ map (filterNTree p) $ filter (\(NTree a _) -> p a) ary
...
treeLoadView reload p = R.createElement el p []
  ...
  cpt { root, mCurrentRoute, session, frontends } _ = do
    useLoader root (loadNode session)
      $ \loaded ->
...
nodeMainSpan d p folderOpen session frontends = R.createElement el p []
  ...
  cpt { id, name, nodeType, mCurrentRoute } _ = do
    ...
    pure
      $ H.span (dropProps droppedFile isDragOver)
          ...
          , H.a
              { href: (url frontends (NodePath nodeType (Just id)))
  ...
  dropHandler (_ /\ setDroppedFile) e =
    unsafePartial
      $ do
          let
            ff = fromJust $ item 0 $ ((e .. "dataTransfer" .. "files") :: FileList)
      ...
      H.div { className: "panel-heading" }
        [ H.div { className: "row" }
            ...
            , H.div { className: "col-md-2" }
                [ H.a
                    { className: "btn text-danger glyphitem glyphicon glyphicon-remove-circle"
...
renameBox d p (true /\ setRenameBoxOpen) = R.createElement el p []
  ...
  cpt { id, name } _ = do
    ...
    pure
      $ H.div { className: "from-group row-no-padding" }
    ...
    renameBtn (newName /\ _) =
      H.a
        { className: "btn glyphitem glyphicon glyphicon-ok col-md-2 pull-left"
        , type: "button"
        , onClick:
          mkEffectFn1
            $ \_ -> do
    ...
    cancelBtn =
      H.a
        { className: "btn text-danger glyphitem glyphicon glyphicon-remove col-md-2 pull-left"
        , type: "button"
