...
fieldsCodeEditorCpt = here.component "fieldsCodeEditorCpt" cpt
  where
    cpt { fields, nodeId } _ = do
      ...
      let
        editorsMap { idx, ftField } =
          fieldCodeEditorWrapper
          { canMoveDown: idx < (List.length fields' - 1)
          , canMoveUp: idx > 0
          , field: ftField
      ...
      pure $
        ...
        $ List.toUnfoldable (editorsMap <$> fields')
...
fieldCodeEditorWrapperCpt = here.component "fieldCodeEditorWrapperCpt" cpt where
  cpt props@{ canMoveDown
            , canMoveUp
            , field: Field { name }
            , onMoveDown
            , onMoveUp
    ...
    -- |
    let
      headerId = "collapse-header--" <> (show nodeId) <> "--" <> (show idx)
      bodyId = "collapse-body--" <> (show nodeId) <> "--" <> (show idx)
      ...
      [
        ...
        ,
          ...
          [
            B.iconButton
            { variant: Secondary
            , callback: const $ onRemove unit
            , name: "trash-o"
            , focusRing: false
            }
          ,
            B.iconButton
            { callback: const $ onMoveUp unit
            , name: "arrow-up"
            , variant: Secondary
            , status: canMoveUp ?
                Enabled $
          ...
          ,
            B.iconButton
            { callback: const $ onMoveDown unit
            , name: "arrow-down"
            , variant: Secondary
            , status: canMoveDown ?
                Enabled $
...
changeCode onc _ CE.JSON c = do
  case JSON.readJSON c of
    Left _ -> here.warn2 "[fieldCodeEditor'] cannot parse json" c  -- TODO Refactor?
    Right j' -> onc $ JSON j'
