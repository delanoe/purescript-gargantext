module Gargantext.Components.Search.SearchField (Search, Props, searchField, searchFieldComponent) where
...
type Search
  = { database :: Maybe Database, term :: String }
...
searchFieldComponent = R.memo (R.hooksComponent "SearchField" cpt) hasChanged
  ...
  cpt props _ = do
    let
      search = maybe defaultSearch identity (fst props.search)
...
submitButton (database /\ _) (term /\ _) (_ /\ setSearch) = button { className: "btn btn-default", type: "button", onClick: click } [ text "Search" ]
  where
  click =
    mkEffectFn1
      $ \_ -> do
