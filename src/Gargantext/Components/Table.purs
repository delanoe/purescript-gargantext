...
type Row
  = { row :: Array R.Element, delete :: Boolean }

type Rows
  = Array Row

type OrderBy
  = Maybe (OrderByDirection ColumnName)

type Params
  = { offset :: Int, limit :: Int, orderBy :: OrderBy }
...
tableCpt = R.hooksComponent "Table" cpt
  ...
  colHeader setOrderBy currentOrderBy c = H.th { scope: "col" } [ H.b {} cs ]
    ...
    cs :: Array R.Element
    cs = case currentOrderBy of
      Just (ASC d)
        | c == d -> [ lnk (Just (DESC c)) "DESC ", lnk Nothing (columnName c) ]
      Just (DESC d)
        | c == d -> [ lnk (Just (ASC c)) "ASC ", lnk Nothing (columnName c) ]
...
textDescription currPage pageSize totalRecords = H.div { className: "row1" } [ H.div { className: "" } [ H.text msg ] ] -- TODO or col-md-6 ?
  where
