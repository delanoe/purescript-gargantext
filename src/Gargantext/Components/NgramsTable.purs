...
ngramsTableSpec session ntype setPath = simpleSpec performAction render
  ...
  performAction (SetParentResetChildren p) _ _ = modifyState_ $ setParentResetChildren p

  performAction (ToggleChild b c) _ _ = modifyState_ $ _ngramsChildren <<< at c %~ toggleMap b
  ...
  performAction (SetTermListItem n pl) { path: { nodeId, listIds, tabType } } { ngramsVersion } = commitPatch session { nodeId, listIds, tabType } (Versioned { version: ngramsVersion, data: pt })
    where
    pe = NgramsPatch { patch_list: pl, patch_children: mempty }

    pt = singletonNgramsTablePatch ntype n pe

  performAction AddTermChildren _ { ngramsParent: Nothing } = -- impossible but harmless
    pure unit

  performAction AddTermChildren { path: { nodeId, listIds, tabType } } { ngramsParent: Just parent
  , ngramsChildren
  , ngramsVersion
  } = do
  ...
  performAction (AddNewNgram ngram) { path: { listIds, nodeId, tabType } } { ngramsVersion } = commitPatch session { listIds, nodeId, tabType } (Versioned { version: ngramsVersion, data: pt })
    where
  ...
  , loaded: Versioned { data: initTable }
  } { ngramsTablePatch, ngramsParent, ngramsChildren } _reactChildren =
    ...
    orderWith = case convOrderBy <$> pageParams.params.orderBy of
    ...
    displayRow (NgramsElement { ngrams, root }) =
      root == Nothing
        ...
        ||
         -- Unless they are scheduled to be removed. ngramsChildren
        ^. at ngrams
        == Just false
...
tree params@{ ngramsTable, ngramsStyle, ngramsEdit, ngramsClick } nd@{ ngrams } =
  ...
  where
  tag = case ngramsClick nd of
    Just effect -> a (ngramsStyle <> [ onClick $ const effect ])
    Nothing -> span ngramsStyle
...
sumOccurrences' ngramsTable label = ngramsTable ^. ix label <<< to (sumOccurrences ngramsTable)

sumOccurrences :: NgramsTable -> NgramsElement -> Additive Int
sumOccurrences ngramsTable (NgramsElement { occurrences, children }) = Additive occurrences <> children ^. folded <<< to (sumOccurrences' ngramsTable)
